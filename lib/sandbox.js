...
function Sandbox( options ) {
  ...
  this.run = function( code, callback ) {
    ...
    child.run = function (code, callback) {
    ...
    function route( msg ) {
      ...
      switch ( msg.type ) {
        ...
        case 'result':
          ...
          if (msg.error) {
            child.kill();
            return callback.call( child, msg.error, msg );
          }
          else {
            return callback.call( child, false, msg );
          }
      }
      return undefined;
    }
    ...
    function onExit( code, msg ) {
      child.kill();
      if ( !hadResult && code !== 0 ) {
        var error = new Error( 'Process exited due to bad state: ' + msg );
        error.code = msg;
        callback.call( child, error , errorAsObject(error) )
      }
    }
